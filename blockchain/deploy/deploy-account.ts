...
export default async function (hre: HardhatRuntimeEnvironment) {
  ...
  const salt = ethers.utils.keccak256(ethers.constants.HashZero);
  ...
  // return
    ...
    console.log("SC Account owner pk: ", eoa.privateKey);

  const deployTx = await aaFactory.populateTransaction.deployAccount(salt, eoa.address);
